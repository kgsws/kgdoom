...
static unsigned char cheat_amap_seq[] = "iddt";
...
( event_t*	ev )
{
    ...
    {
	if(
		(ev->type == ev_keydown && ev->data1 == AM_STARTKEY) ||
		(ev->type == ev_joystick && ev->data1 & (1 << i_ctrl_btn[joymap]))
	) {
	    AM_Start ();
	    viewactive = false;
	    rc = true;
    ...
    if(ev->type == ev_joystick && ev->data1 & (1 << i_ctrl_btn[joymap]))
    {
	bigstate = 0;
	viewactive = true;
	AM_Stop ();
...
void AM_drawMarks(void)
{
	...
	if (markpoints[i].x != -1)
	{
		...
		V_DrawPatchNew(fx, fy, marknums[i], v_colormap_normal, V_HALLIGN_NONE, V_VALLIGN_NONE, 2);
	}
    }
