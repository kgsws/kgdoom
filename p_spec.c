  ...
  mobj_t*	thing )
{
    ...
    if(isHexen)
    {
	P_ExtraLineSpecial(thing, &lines[linenum], side, EXTRA_CROSS);
	return;
    }
...
void P_UpdateSpecials (void)
{
	...
	if (buttonlist[i].btimer)
	{
		...
		if(info)
		{
		    ...
		    SV_StartLineSound(buttonlist[i].line, sfx_swtchn);
		}
#else
...
boolean P_ExtraLineSpecial(mobj_t *mobj, line_t *line, int side, int act)
{
	...
	backsector = side ? line->frontsector : line->backsector;

	switch(special)
	{
		case 10: // Door_Close
			EV_GenericDoor(backsector, line->arg[0], line->arg[2], line->arg[1], VDOORWAIT_SINGLE, -1);
		break;
		case 11: // Door_Open
			EV_GenericDoor(backsector, line->arg[0], line->arg[2], line->arg[1], VDOORWAIT_SINGLE, 1);
		break;
		case 12: // Door_Raise
			EV_GenericDoor(backsector, line->arg[0], line->arg[3], line->arg[1], line->arg[2], 1);
		break;
		default:
			sprintf(temp_text, "UseSpecial: %i(%i, %i, %i, %i, %i)", special, line->arg[0], line->arg[1], line->arg[2], line->arg[3], line->arg[4]);
			printf("%s\n", temp_text);
			players[consoleplayer].message = temp_text;
		return false;
	}

	P_ChangeSwitchTexture(line, line->flags & ELF_ACT_REPEAT);
