...
void WI_drawLF(void)
{
    ...
    // draw <LevelName> 
    V_DrawPatch((320 - SHORT(lnames[wbs->last]->width))/2,
		y, FB, lnames[wbs->last]);

    // draw "Finished!"
    y += (5*SHORT(lnames[wbs->last]->height))/4;
    
    V_DrawPatch((320 - SHORT(finished->width))/2,
		y, FB, finished);
}
...
void WI_drawEL(void)
{
    ...
    // draw "Entering"
    V_DrawPatch((320 - SHORT(entering->width))/2,
		y, FB, entering);

    // draw level
    y += (5*SHORT(lnames[wbs->next]->height))/4;

    V_DrawPatch((320 - SHORT(lnames[wbs->next]->width))/2,
...
void WI_drawStats(void)
{
    ...
    if (wbs->epsd < 3)
    {
	V_DrawPatch(320/2 + SP_TIMEX, SP_TIMEY, FB, par);
